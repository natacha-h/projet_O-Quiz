...
# sh import-external-repo.sh git@github.com:O-clock-XXXXX/nom-de-depot.git nom-de-la-branche-a-creer
#
# l'argument "git@github.com:O-clock-XXXXX/nom-de-depot.git" correspond au lien permettant de cloner le dépôt
# l'argument "nom-de-la-branche-a-creer" correspond au nom de la branche dans laquelle les sources seront récupérées (on écrase pas les sources actuelles)

if [ $# -eq 0 ] || [ $# -eq 1 ]
then
    echo " "
    echo "Usage: sh import-external-repo.sh git@github.com:O-clock-XXXXX/repository-name.git branch-name-to-create"
    echo " "
    exit 0
fi
